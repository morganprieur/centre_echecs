...
class Match(): 
    ...
    def serialize_multi_matches(self): 
        """ Serialization of the matches data in order to register them 
            in the DB. 
        Args:
            matches (list of objects): list of object Matches_start 
        Returns:
            serialized_matches_start (list of dicts): the matches in the expected format for the DB 
        """ 
        print(f'matches_obj_start MM50 : {self.matches_obj_start}') 
        serialized_matches_start = [] 

        # print(f'matches_obj_start M63 : {matches_obj_start}') 
        for m_obj in range(len(self.matches_obj_start)): 
            # print(f'type(p_obj) : {type(p_obj)}\n') 
            # print(f'p_obj : {p_obj}\n') 
            # print(f'players[{p_obj}] : {players[p_obj]}\n') 
            serialized_match_start_data = {
                'tuple_match': self.matches_obj_start[m_obj].tuple_match 
            } 
    ...
    def instantiate_scores_round_1(scores_players_round_1): 
        print(f'scores_round_1 MM75 : {scores_players_round_1}') 
        ...
        print(f'matches_round_1 MM89 : {matches_round_1}') 
